...
def test_guarantee_exists():
    ...
    with doo.io_utils.clear_file_after(filename):
        doo.io_utils.guarantee_exists(filename)
...
def test_guarantee_dir_exists():
    filename = DEFAULT_FILE_NAME
    with doo.io_utils.clear_file_after(filename):
        doo.io_utils.guarantee_dir_exists(filename)
...
def test_json_dumps_numpy_array():
    assert doo.io_utils.json_dumps(np.array([4])) == "[4]"
...
def test_json_dumps_numpy_array_element():
    ...
    """
    assert doo.io_utils.json_dumps(np.array([4])[0]) == "4"
    assert doo.io_utils.json_dumps(np.array([4], dtype=np.int64)[0]) == "4"


def test_yaml_loading_and_dumping():
    # writing to a string
    arr = np.array([1, 2, 3])
    nt.eq_(doo.io_utils.yaml_loads(doo.io_utils.yaml_dumps(arr)),
           [1, 2, 3])
    ...
    doo.io_utils.yaml_dump(np.array([4, 5, 6]), filename)
    nt.eq_(doo.io_utils.yaml_load(filename),
           [4, 5, 6])
...
def test_tmp_chmod_no_write():
    ...
    try:
        ...
        with doo.io_utils.tmp_chmod_no_write(filename):
...
def test_atomically_writable_file():
    ...
    try:
        with doo.io_utils.tmp_chmod_no_write(filename):
            ...
            assert read_file() == ""
            with doo.io_utils.atomically_writable_file(
                    filename,
