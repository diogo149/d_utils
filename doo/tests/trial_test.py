...
from doo._test_utils import equal, hash_equal
...
def in_trials_dir(*ps):
    return os.path.join(doo.config["trial"]["trials_dir"], *ps)


def test_trial_exception():
    with doo.io_utils.clear_file_after(in_trials_dir()):
        try:
            with doo.trial.run_trial(
                    "foobar",
...
def test_trial_db():
    with doo.io_utils.clear_file_after(in_trials_dir()):
        ...
        with doo.trial.trial_db_iteration_transaction(
...
def test_trial_delete():
    with doo.io_utils.clear_file_after(in_trials_dir()):
        with doo.trial.run_trial("foobar", 55):
            pass

        with doo.trial.run_trial("foobar", 100):
            pass

        with doo.trial.trial_db_transaction(in_trials_dir("foobar")) as db:
            assert len(db) == 2
            assert db[0]["iteration_num"] == 55
            assert db[1]["iteration_num"] == 100

        doo.trial.TrialState("foobar", 55).delete()
...
def test_trial_random_seed():
    ...
    with doo.io_utils.clear_file_after(in_trials_dir()):
        with doo.trial.run_trial("foobar", random_seed=1):
            foo()

        with doo.trial.run_trial("foobar", random_seed=1):
            foo()

        with doo.trial.run_trial("foobar", random_seed=2):
            foo()
...
def test_run_trial_function():
    ...
    with doo.io_utils.clear_file_after(in_trials_dir()):
        doo.trial.run_trial_function(trial_fn,
                                     trial_name="foobar",
                                     iteration_num=1)
...
import doo
time.sleep(2)
with doo.trial.run_trial("thisisatest"):
    pass
"""
    with doo.io_utils.temporary_directory("__race_condition"):
        with doo.io_utils.chdir("__race_condition"):
